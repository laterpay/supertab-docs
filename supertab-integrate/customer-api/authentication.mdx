...
**Supertab currently only supports authenticating customers through the use of a pop up**, you must pass the
redirect URI as `https://signon.supertab.co/oauth/auth-proxy?origin={your-site-url}` where `{your-site-url}` is the
url you set when creating the site.
