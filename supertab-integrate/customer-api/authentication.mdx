...
[Customer SDK](/dead) to obtain tokens.

To make authenticated calls pass the token acquired from the Token URL in the `Authorization` header.

<CodeGroup>
    ```bash curl
    curl --location 'https://tapi.supertab.co/customer/customers/me' \
    --header 'Authorization: Bearer ••••••
    ```

    ```javascript javascript
    fetch('https://tapi.supertab.co/customer/customers/me', {
        method: 'GET',
        headers: {
...
We recommend you use the Customer SDK to handle all authentication, which takes care of this for you.
...
The [Customer SDK](/dead) handles acquiring tokens with the correct scopes for you.