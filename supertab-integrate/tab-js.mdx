...
To get started with purchase button, first <a href="../supertab-experiences/experiences">set up the experience</a>. The code snippet generated in the last step of experience editor will look like the below.
...
<script type="module">
  ...
  (async () => {
    const { destroy } = await createPurchaseButtonExperience({
      ...
      offeringId: "offering.77bdfc93-5de0-48da-a9d0-65754e0bb947", // optional
      onPurchaseCompleted: () => {
...
<ParamField path="onError" type="function">
  ...
  continue. For example when wrong client ID and offering ID is supplied, or
