...
<ResponseField name="toggle" type="function">
    Toggle visibility of the paygate UI.

    Returns a promise which resolves with the paygate summary.
    <Info>
        If current user has a prior entitlement to the content, the paygate will not
        show.
    </Info>
...
## Purchase Button `supertab.startPurchaseButton`
