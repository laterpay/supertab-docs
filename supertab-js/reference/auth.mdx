...
This property returns the authentication status of the user. See [AuthStatus](#authstatus) for the type definition.
...
<ParamField path="silently" type="boolean" default="false">
  ...
  instead attempts to refresh the session in the background. This will make the
  method return `null` for unknown users with no prior session.
</ParamField>
